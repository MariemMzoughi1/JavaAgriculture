...
public class CultureService {
    ...
    public CultureService() {
        connection = DbConnection.getInstance().getConn(); // ou DataSource.getInstance().getConnection() selon ton projet
    }
    ...
    // ❌ Supprimer une culture (et ses parcelles associées)
    ...
    // 🌾 Estimation de récolte par culture
    public double estimerRecolteParCulture(String nomCulture, double superficieHa, String saison) {
        double rendementBase = switch (nomCulture.toLowerCase()) {
            case "blé" -> 5.0;
            case "orge" -> 4.0;
            case "tomate" -> 40.0;
            case "pomme de terre" -> 35.0;
            case "carotte" -> 30.0;
            case "pastèque" -> 20.0;
            default -> 10.0;
        };
    ...
    // 💡 Suggérer des cultures selon type de sol et saison
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                boolean compatible = switch (typeSol.toLowerCase()) {
                    ...
                    case "sableu" -> categorie.contains("légume") || categorie.contains("racine");
    ...
    // 📊 Obtenir la quantité totale par catégorie

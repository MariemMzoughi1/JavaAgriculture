...
public class PostService implements InterfaceCRUD<Post> {
    ...
    public void delete(Post post) {
        try {
            ...
            try (PreparedStatement ps1 = con.prepareStatement(deleteComments)) {
                ps1.setInt(1, post.getId());
                ps1.executeUpdate();
            }

            // Supprimer le post
            String deletePost = "DELETE FROM post WHERE id = ?";
            try (PreparedStatement ps2 = con.prepareStatement(deletePost)) {
    ...
    public User getAuteurByPostId(int postId) {
        String sql = "SELECT u.id, u.email, u.password, u.role, u.reset_token " +
                "FROM post p JOIN users u ON p.auteur_id = u.id WHERE p.id = ?";
        try (PreparedStatement pst = con.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                return new User(
                        ...
                        rs.getString("role"),
                        rs.getString("reset_token") // ajoute ce champ dans la requête SQL ci-dessous
                );
            }
        } catch (SQLException e) {
    ...
    public boolean maj(Post post) {
        String sql = "UPDATE post SET titre = ?, contenu = ? WHERE id = ?";
